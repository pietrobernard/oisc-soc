...
entity subbus_dev_v2 is
    ...
    port (
        ...
        bus_err_sys: in std_logic;          --  this line will go high if the booking fails
        ...
        dbg_devbus: out natural;
        dbg_bus_mode: out std_logic_vector(1 downto 0)
    );
end subbus_dev_v2;

architecture Behavioral of subbus_dev_v2 is
    --  constants    
    ...
    signal s_bus_rdy_sys: std_logic;
    signal s_bus_err_sys: std_logic;
    ...
    --  STATE MACHINE
    type t_SM is (  s_INIT, s_IDLE, s_LATCH_ERR, s_ADDR_CHECK_INT, s_ADDR_CHECK_BUS, s_WAIT, s_TRAP,
                    s_M_0, s_M_1, s_M_2, s_M_3, s_M_4, s_M_5,
                    s_MB_0, s_MB_1, s_MB_2, s_MB_3,
    ...
    signal r_master_jump: t_SM := s_INIT;
                ...
                begin
                    if (rising_edge(sysClk)) then
                        case (r_stage) is
                            ...
                            when s_MB_0 =>
                                s_bus_rdy_sys <= bus_rdy_sys;
                                s_bus_err_sys <= bus_err_sys;
                                ss_sync_0 <= '1';
                                ss_sync_1 <= '0';
                            ...
                            case (r_stage) is
                                ...
                                when s_R_asS_1 =>
                                    ...
                                    if (ss_reg_drdy='1') then
                                        ...
                                        r_bus_address_to <= std_logic_vector(to_unsigned(addr, addr_width));
                                ...
                                when s_VRT_1 =>
                                    r_dbg_stage <= 16;
                                    --r_dbg_stage <= to_integer(unsigned(s_dev_in_data));
                                    if (s_dev_in_latch='1') then
                                ...
                                when s_M_0 =>
                                    ...
                                    if ((ss_sync_1='1') and (s_dev_in_latch='1')) then
                                        ...
                                        r_bus_data_to <= s_dev_in_data;
                                        r_bus_keep <= s_dev_in_keep;                                                                               
                                        r_stage <= r_master_jump;   --  initially set to s_MB_0 to book the bus
                                    else
                                        --  waiting
                                        r_stage <= s_M_0;
                                    end if;
                                                                
                                --  MASTER BOOKING STAGE (FOR THE EXT-BUS CONNECTED TO THE OTHER END OF THE BRIDGE)
                                --  Now we have to try to book the main-bus over to the bridge, let's try to do that                                                      
                                when s_MB_0 =>
                                    r_dbg_stage <= 20;
                                    check := (ss_sync_0 & s_bus_rdy_sys & s_bus_err_sys);
                                    case (check) is
                                        when "100" =>
                                            --  waiting for booking
                                            r_bus_req_sys <= '1';
                                        ...
                                        when "101" =>
                                            --  impossible to book right now
                                            r_stage <= s_MB_1;
                                        
                                        when "110" =>
                                            --  booking is ok, we can release the request
                                            r_stage <= s_MB_2;
                                ...
                                when s_MB_1 =>
                                    r_dbg_stage <= 21;
                                    --  must release the booking attempt
                                    if (s_bus_err_sys='0') then
                                        --  now going to release
                                        r_stage <= s_LATCH_ERR;
                                    else
                                        --  renouncing
                                        r_bus_req_sys <= '0';
                                ...
                                when s_MB_2 =>
                                    r_dbg_stage <= 22;
                                    --  we can now release the bus req sys
                                    if (s_bus_rdy_sys='0') then
                                        --  can now lower my request, this will make the bridge switch into INT 2 eXT MODE
                                        r_bus_req_sys <= '0';
                                        --  now setting the jump so that subsequent transaction can bypass the booking procedure
                                        r_master_jump <= s_M_1;
                                        r_stage <= s_M_1;
                                    else
                                        r_stage <= s_MB_2;
                                ...
                                when s_M_2 =>
                                    r_dbg_stage <= 24;
                                    --  termination of request, so that we can try later
                                    --  r_master_jump is NOT resetted, so that if a further attempt is made, then
                                    --  the ext bus will remain booked.
