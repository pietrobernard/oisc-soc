...
architecture Behavioral of subdev is
    --  constants
    constant dev_mem_begin: integer := local_mem_begin;
    constant dev_mem_end: integer := local_mem_begin + 63 + local_mem_nvrt;
                    ...
                    if (sysRstb='0') then
                        ...
                        case (r_stage) is
                            ...
                            when s_toBUS_1 =>
                                --  debug
                                r_dbg_stage <= 3;
                                --  now we must wait for the device to output the data, so
                                if (ss_dev_out_drdy='1') then
                                    --  here's the response from the remote slave that must be forwared to the wrapper
                            ...
                            when s_toBUS_2 =>
                                ...
                                if (s_int_dev_in_latch='0') then
                                    --  the interface has acknowledged the data, and now we need the response                                    
                                    r_dev_out_drdy <= '0';
                                    r_dev_in_latch <= '0';
                                    r_stage <= s_toBUS_3;
                                else
                            ...
                            when s_toBUS_3 =>
                                ...
                                if (ss_dev_out_drdy='0') then
                                    --  the bus trx has lowered drdy.
                                    --  this means that we know have to take some actions
                                    if (ss_dev_out_done='0') then
                                        --  we still have to catch more data, so:
                                        r_jump <= s_toBUS_1;
                                        r_stage <= s_toBUS_4;
                                    else
                                        --  no more data to catch from the slave, so
                                        r_dev_out_done <= '0';
                            ...
                            when s_toBUS_5 =>
                                --  debug
                                r_dbg_stage <= 7;
                                --  here we must wait for another bus signal
                                if (s_int_dev_in_latch='1') then
                                    --  setting the inputs
                                    r_dev_in_cmd <= s_int_dev_in_cmd;
                                    r_dev_in_addr <= s_int_dev_in_addr;
