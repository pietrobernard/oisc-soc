...
entity subdev_UART_dev1_v2 is
    ...
    port (
        ...
        bus_err_sys: in std_logic;          --  this line will go high if the book fails
        --  irq lines
...
end subdev_UART_dev1_v2;

architecture Behavioral of subdev_UART_dev1_v2 is
    --  subdev interface
    ...
    --  state machine
    type t_SM is (s_INIT, s_IDLE, s_LREG_0, s_VREG_0, s_VREG_1,s_VREG_2, s_VREG_3, s_VREG_4, s_VREG_5, s_VREG_6, s_HW_0, s_HW_1, s_HW_2, s_HW_3, s_HW_4, s_HW_5, s_HW_6, s_HW_7, s_HW_8,
    s_COLL_0, s_COLL_1);
                            ...
                            case (r_stage) is
                                ...
                                when s_HW_5 =>
                                    ...
                                    if (ss_irq_grant='1') then
                                        ...
                                        r_irq_data(27 downto 5) <= std_logic_vector(to_unsigned(local_mem_begin+64, addr_width));   --local_mem_begin+64
                                ...
                                when s_HW_7 =>
                                    r_dbg <= 8;                                    
                                    --  si blocca sempre qui, vediamo lo stato di errore
                                    ...
                                    case (cond) is
                                        ...
                                        when "001"|"011" =>
                                            --  error occurred during bus attempt
                                            --  this is because the bus is still not ready or there could be a collision
                                            r_stage <= s_COLL_0;
                                ...
                                when s_VREG_4 =>
                                    r_dbg <= 15;
                                    if ((ss_sync_0='1') and (ss_bus_out_drdy='1')) then
                                ...
                                --  this state is hypothetical (should not happen)
                                --  in case a collision occurrs while handling interrupts
                                when s_COLL_0 =>
                                    r_dbg <= 18;
                                    cond := ss_sync_0 & ss_bus_out_drdy & ss_bus_err;
                                    case (cond) is
                                        when "100" =>
                                        ...
                                        when "101"|"111" =>
                                            --  let's lower the latch
                                            r_bus_in_latch <= '0';
                                            r_stage <= s_COLL_0;
                                        
                                        when "110" =>
                                            --  this is not due to our latch, it is incoming data colliding with ours
                                            r_stage <= s_COLL_1;
                                        
                                        when others =>
                                            --  impossible state or not yet ready
                                            r_stage <= s_COLL_0;
                                    end case;
                                
                                when s_COLL_1 =>
                                    r_dbg <= 19;
                                    --  dumping colliding data
                                    if (ss_bus_out_drdy='0') then
                                        --  done
                                        r_bus_in_latch <= '0';
                                        r_stage <= s_COLL_0;
                                    else
                                        --  giving up collision data
                                        r_bus_in_latch <= '1';
                                        r_stage <= s_COLL_1;
                                    end if;
